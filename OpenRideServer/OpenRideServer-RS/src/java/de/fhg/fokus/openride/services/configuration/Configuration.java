...
public class Configuration {
    ...
    private String imgPath = "/OpenRideWeb/img/profile/";
    private String imgFullPath = "C:\\Users\\Omri\\Documents\\NetBeansProjects\\OpenRideServer\\OpenRideWeb\\web\\img\\profile/";

    @GET
    @Produces("text/json")
    @Path("init")
    public Response getConfiguration(@Context HttpServletRequest request) throws ServletException, IOException {
        ...
        if (nickname != null) {
            ...
            newratings = 0;
            String img = null;
            File image = new File(imgFullPath + customer.getCustNickname() + "_" + customer.getCustId() + ".jpg");
            if (!image.exists())
            {
                img = imgPath + "default.jpg";
            try
            {
            PrintWriter writer = new PrintWriter("next-file.txt", "UTF-8");
            writer.println(request.getPathInfo());
            writer.println(imgPath+customer.getCustNickname() + "_" + customer.getCustId()+".jpg");
            writer.close();
            }
            catch(Exception e){}
            }
            ...
            {
                img = imgPath + customer.getCustNickname() + "_" + customer.getCustId() + ".jpg";
            try
            {
            PrintWriter writer = new PrintWriter("next-new-file.txt", "UTF-8");
            writer.println(request.getPathInfo());
            writer.close();
            }
            catch(Exception e){}
            }
         //   else img = imgPath + "maxAm.jpg";
    ...
    public Response getUpdates(@Context HttpServletRequest request) {
        System.out.println("getUpdates" + request.getRemoteUser());
        String nickname = request.getRemoteUser();
        if (nickname != null) {
